...
API_ID = int(environ.get('API_ID', '21001356'))
API_HASH = environ.get('API_HASH', 'dfba8e798dbc7e0174d4f633462ffa64')
BOT_TOKEN = environ.get('BOT_TOKEN', "8010128258:AAEQkxifUuuygNAqx_2pbfcwgmEvjHZ9Byc")
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7972685235').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002424190826').split()]  # Channel id for auto indexing (make sure bot is admin)
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002424190826')  # Channel / Group Id for force sub (make sure bot is admin)
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '')  # support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002424190826')  # request channel id (make sure bot is admin)
...
# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://savindulasenehas:R3TXj5zozKgJ7Ph8@savi.srkg5.mongodb.net/?retryWrites=true&w=majority&appName=savi")
DATABASE_NAME = environ.get('DATABASE_NAME', "savi")
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002251563134'))  # Log channel id (make sure bot is admin)
...
# Default value fix for PREMIUM_LOGS
try:
    PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '0'))  # Default value '0' if not set
except ValueError:
    PREMIUM_LOGS = 0  # In case of error, fallback to '0'
